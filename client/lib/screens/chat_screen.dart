...
class _ChatScreenState extends State<ChatScreen> {
  ...
  void _onCallEvent(centrifuge.PublicationEvent event) {
    ...
    if (ev == 'call.connected') {
      ...
      if (livekitUrl != null && token != null) CallService().onCallConnected(livekitUrl, token);
  ...
  Future<void> _startCall() async {
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Qo'ng'iroq xatosi: $e")));
    }
  }
  ...
  void _updateTabTitle(int unread) {
    if (!kIsWeb) return;
    try {
      html.document.title = unread > 0 ? '($unread) Nova Chat' : 'Nova Chat';
    } catch (_) {}
  }
  ...
  Future<void> _send(ChatProvider chat, String? roomId) async {
    ...
    if (_selectedFile != null) {
      await _sendFile(chat, roomId, _selectedFile!);
      return;
    }
  ...
  Future<void> _sendFile(ChatProvider chat, String roomId, PlatformFile file) async {
    ...
    try {
      final att = await MediaService().upload(
        file,
      ...
      child: Row(
        ...
        children: [
          ConstrainedBox(
            ...
            child: Container(
              padding: EdgeInsets.symmetric(horizontal: isMedia ? 6 : 14, vertical: isMedia ? 6 : 10),
              decoration: BoxDecoration(
...
class _MediaContentState extends State<_MediaContent> {
  ...
  Widget _buildImage(AttachmentInfo info) {
    return GestureDetector(
      ...
      child: ClipRRect(
        ...
        child: Image.network(
          ...
          width: 220,
          fit: BoxFit.cover,
          errorBuilder: (_, __, ___) => const SizedBox(width: 220, height: 80,
  ...
  Widget _buildAudio(AttachmentInfo info) {
    return GestureDetector(
      onTap: () => _openUrl(info.url),
